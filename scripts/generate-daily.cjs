...
// --- Load used.json ledger ---
const usedPath = path.resolve('used.json');
let used = [];
if (fs.existsSync(usedPath)) {
  try {
    used = JSON.parse(fs.readFileSync(usedPath, 'utf-8'));
  } catch (e) {
    console.warn("Could not parse used.json, starting fresh");
    used = [];
  }
}
...
async function fetchQuestions(retries = 3) {
  const url = 'https://opentdb.com/api.php?amount=10&type=multiple'; // grab extras
...
(async () => {
  ...
  try {
    ...
    // Build and filter out already-used questions
    let questions = results.map((q, idx) => {
    ...
    const fresh = questions.filter(q => !used.includes(q.text)).slice(0, 5);

    // If somehow <5 remain, just fallback to first 5
    const finalQuestions = fresh.length >= 5 ? fresh : questions.slice(0, 5);
